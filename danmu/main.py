...
list_name = 'bilibili'
key_list = ('w', 's', 'a', 'd', 'j', 'k', 'u', 'i', 'r', 'z', 'x', 'f', 'enter', 'shift', 'space'
            , 'up', 'down', 'left', 'right')
...
async def printer(q, redis):
    while True:
        ...
        if m['msg_type'] == 'danmaku':
            ...
            list_str = m["content"]
            print("弹幕拆分:", list_str)
            if list_str.lower() in key_list:
                    print('推送队列：', list_str.lower())
                    redis.rpush(list_name, list_str.lower())
            #     原代码(根据你的实际情况修改)
...
#a = input('请输入直播间地址：\n')
# 下面填入你的直播间地址
a = "https://live.bilibili.com/21577989"
asyncio.run(main(a))
